...
class EndGame(Page):
    ...
    def vars_for_template(self):
        ...
        if self.player.participant.vars["number_of_timeouts"] > Constants.num_rounds*Constants.timeout_submission_rate_threshold:
            final_payoff = 0.0 + Constants.penalty # gets a reduced payment
            penalized = True
        else:
            final_payoff = round(self.player.participant.vars["payoff_in_paying_round"],2)
            penalized = False
        ...
        return {
                ...
                'paying_round_payoff': round(self.player.participant.vars["payoff_in_paying_round"],2),
